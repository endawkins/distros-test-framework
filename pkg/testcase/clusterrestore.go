...
func takeS3Snapshot(
	...
	takeSnapshotCmd := fmt.Sprintf("sudo %s etcd-snapshot save --s3 --s3-bucket=%s "+
		...
		cluster.Config.Product, flags.S3Flags.Bucket, flags.S3Flags.Folder, cluster.Aws.Region,
...
func installProduct(
	...
	if cluster.Config.Product == "k3s" {
		...
		installCmd = installCmd + fmt.Sprintf("https://get.%s.io | sudo INSTALL_%s_VERSION=%s sh -",
			cluster.Config.Product, strings.ToUpper(cluster.Config.Product), version)
	}

	res, installCmdErr := shared.RunCommandOnNode(installCmd, newClusterIP)
...
func enableAndStartService(
	...
	time.Sleep(120 * time.Second)
	_, statusServiceCmdErr := shared.ManageService(cluster.Config.Product, "status", "server",
		[]string{newClusterIP})
...
func postValidationRestore(cluster *shared.Cluster, newServerIP string) {
	...
	var getNodesPodsCmd string
	if cluster.Config.Product == "rke2" {
		getNodesPodsCmd = fmt.Sprintf("export KUBECONFIG=/etc/rancher/%s/%s.yaml && PATH=$PATH:/var/lib/rancher/%s/bin "+
			" && /var/lib/rancher/%s/bin/kubectl get nodes,pods -A -o wide  ",
			cluster.Config.Product, cluster.Config.Product, cluster.Config.Product, cluster.Config.Product)
	} else {
		getNodesPodsCmd = fmt.Sprintf("export KUBECONFIG=/etc/rancher/%s/%s.yaml && PATH=$PATH:/var/lib/rancher/%s/bin "+
			" && /usr/local/bin/kubectl get nodes,pods -A -o wide  ",
			cluster.Config.Product, cluster.Config.Product, cluster.Config.Product)
	}
	...
	// VALIDATE IF THE NEWLY CREATED NODE IS READY.
	// VALIDATE PODS STATUS.
	// validate if previous deployments and newly created deployments are running.
